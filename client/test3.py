...
class CatalogView(TemplateView):
    ...
    def post(self, request):
        ...
        shipping_method = request.POST.get('shipping_method_id',None)
...
class ProductDetailView(CatalogView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        shipping_metehod = ShippingMethod.objects.filter(is_active=True)
        product_varients = ProductVarient.objects.filter(product=product).order_by('display_order_varient')
        ...
        try:
            ...
            for item in product_varients:
                attributes = []
                varient_attributes = AttributeValue.objects.filter(attribute_value=item)
                attributes = [attr.value for attr in varient_attributes]
                search_obj.append({'varient_id':item.id,'attribute':attributes,'old_price':item.old_price,'new_price':item.price,'discount':item.discount()})
            ...
            #     if related_values:
            #         first_varient_id = ProductVarient.objects.filter(product=product, attribut_value__value=value).filter(product=product,attribut_value__value=related_values[0])[0].id
            #         first_varient = ProductVarient.objects.filter(product=product, attribut_value__value=value).filter(product=product, attribut_value__value=related_values[0])
            #     else:
            ...
            #                                               first_varient=first_varient,search=search_obj,shipping_method=shipping_metehod)
            # else:
            return super(ProductDetailView, self).get(request, product=product,images=images,first_varient=first_varient,search=search_obj,shipping_method=shipping_metehod)

        except Exception as e:
            return super(ProductDetailView, self).get(request, product=product, images=images,first_varient=first_varient,shipping_method=shipping_metehod)
...
def placeOrder(request):
   ...
   if request.method=='POST':
      ...
      response = validate_order_request(receiver_fullname,receiver_address,receiver_number1,sender_full_name,sender_phone_number,receiver_email,sender_email,payment_method)
      ...
      if float(total) < float(settings_object.minimum_order_price):
         ...
         return JsonResponse({'error':'Minimum cart requirement is a subtotal of '+ str(settings_object.minimum_order_price)})
      ...
      address=DeliveryAddress.createDeliveryAddress(i_am_receiver,receiver_fullname,
         ...
         receiver_number2,occasion,sender_full_name,sender_phone_number,sender_email,hide_info_from_receiver)
      ...
      if address:
         if not request.user.is_authenticated:
            ...
            for item in sale_obj:
               OrderExtraCharge.objects.create(order=order,name=item['name'],rate=item['rate'],total=item['value'])
            ...
            for item in data.get('cart_list'):
               ...
               if addons and len(addons)>=1:
                  ...
                  for i in addons:
                     ...
                     AddonOrderItem.objects.create(order_item=order_item,addons=addon,quantity=addons_quantity[a])
                     a=a+1
